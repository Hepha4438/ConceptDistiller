...
def composite_and_save(img_rgb_uint8, cams_list, out_path, cmap="jet"):
    H, W = img_rgb_uint8.shape[:2]
    orig = Image.fromarray(img_rgb_uint8)

    cam_ims = []
    for cam in cams_list:
        cam_rgb = heatmap_apply_colormap(cam)
        cam_rgb = Image.fromarray(cam_rgb).resize((W, H), Image.BICUBIC)
        cam_ims.append(cam_rgb)

    total_w = W * (1 + len(cam_ims))
...
def generate_gradcam_for_best(model, best_obs, frames, out_dir, device="cpu", fps=6):
    ...
    for i, obs_raw in enumerate(best_obs):
        ...
        try:
            img_input, cams = compute_concept_gradcams(fx, obs_t, device)
        ...
        composite_and_save(orig, cams, out_file)
    ...
    try:
        ...
        vid_path = None
        print("Warning: moviepy not installed — skipping video.")
