...
class ModelTesterUI:
    ...
    def run_training(self, algo, hyperparams):
        ...
        try:
            ...
            if algo == "DQN":
            ...
            elif algo == "SDT":  # Soft Decision Tree
                from stable_baselines3.common.vec_env import DummyVecEnv, VecMonitor
                from stable_baselines3.common.callbacks import CheckpointCallback, EvalCallback
                from train_sdt import make_env, SDTPolicy, SDTLoggingCallback
                from minigrid_features_extractor import MinigridFeaturesExtractor
                
                n_envs = hyperparams.pop('n_envs', 4)
                n_concepts = hyperparams.pop('n_concepts', 4)
                concept_mode = hyperparams.pop('concept_mode', 5)
                lambda_1 = hyperparams.pop('lambda_1', 0.05)
                lambda_2 = hyperparams.pop('lambda_2', 0.004)
                lambda_3 = hyperparams.pop('lambda_3', 2.0)
                constraint_lambda = hyperparams.pop('constraint_lambda', 1.0)
                n_continuous_concepts = hyperparams.pop('n_continuous_concepts', 1)
                tree_depth = hyperparams.pop('tree_depth', 4)
                tree_temperature = hyperparams.pop('tree_temperature', 1.0)
                penalty_coef = hyperparams.pop('penalty_coef', 0.01)
                ...
                # Create callbacks
                sdt_logging_cb = SDTLoggingCallback()
                ...
                from stable_baselines3.common.callbacks import CallbackList
                callback = CallbackList([sdt_logging_cb, checkpoint_callback, eval_callback, progress_callback, stop_callback])
